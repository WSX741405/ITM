...
void UIEngine::glutDisplayFunction()
{
	...
	{
		sprintf(str, "colours (currently %s) \t", uiEngine->colourModes_freeview[uiEngine->currentColourMode].name);
	}
	else
	{
		sprintf(str, "colours (currently %s) \t", uiEngine->colourModes_main[uiEngine->currentColourMode].name);
	}
...
void UIEngine::glutKeyUpFunction(unsigned char key, int x, int y)
{
	...
	case 'b':
	{
		...
		uiEngine->imageSource->setBackground();
		printf("done\n");
		break;
	}
	case 'c':
