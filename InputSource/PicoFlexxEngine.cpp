// Copyright 2017 Akos Maroy and the authors of ITM
...
public:
	...
	vector<short> backgroundImage;
	vector<short> depthImage;
	vector<short> objImage;
	mutex mtx;
...
void PicoFlexxEngine::PrivateData::onNewData(const DepthData *data)
{
	...
	objImage.clear();
	objImage.reserve(data->points.size());
	...
	for (size_t pointId = 0; pointId < data->points.count(); pointId++)
	{
		...
		short d = dd.depthConfidence > 128 ? (short)(dd.z * 1000.0) : 0;
		depthImage.push_back(d);
		//	 remove background
		if (backgroundImage.size() != 0)
		{
			short SH = 50;
			if (depthImage[pointId] >= backgroundImage[pointId] - SH &&  depthImage[pointId] <= backgroundImage[pointId] + SH)
			{
				objImage.push_back(0);
			}
			else
			{
				objImage.push_back(depthImage[pointId]);
			}
		}
	}
}
...
void PicoFlexxEngine::getImages(ITMUChar4Image *rgbImage, ITMShortImage *rawDepthImage)
{
	...
	// remove background
	if (data->objImage.size()) memcpy(depth, data->objImage.data(), rawDepthImage->dataSize * sizeof(short));
	else if (data->depthImage.size()) memcpy(depth, data->depthImage.data(), rawDepthImage->dataSize * sizeof(short));
	else memset(depth, 0, rawDepthImage->dataSize * sizeof(short));
}
...
bool PicoFlexxEngine::setBackground()
{
	if (data->depthImage.size() == 0)	return false;
	lock_guard<mutex> lock(data->mtx);
	data->backgroundImage.clear();
	data->backgroundImage.reserve(data->depthImage.size());
	data->backgroundImage = data->depthImage;
	return true;
}
